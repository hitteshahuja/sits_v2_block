 ...
 * @package    blocks
 * @subpackage sits
 * @copyright  2014 University of Bath
...
function sits_autoloader($class){
	global $CFG;
	include $CFG->dirroot.'/blocks/sits_v2/forms/'.$class.'.php';
}
spl_autoload_register('sits_autoloader');
require_login();
if (isguestuser()) {
...
<?php 
//Get the period slot codes from the database 
$periodslotcodes = $DB->get_records('sits_period');
var_dump($periodslotcodes);
$sitsperiodslotcode = new sits_periodslotcode_form();
$sitsperiodslotcode->display();
?>
